...
with col1:
    ...
    ax.set_title('Phân phối ngân sách (USD)', fontsize=16)
...
with col2:
    ...
    ax.set_title('Phân phối doanh thu (USD)', fontsize=16)
...
with col1:
    ...
    ax.set_title('Tương quan giữa số lượt bình chọn và đánh giá trung bình', fontsize=16)
...
with col2:
    ...
    ax.set_title('Tương quan giữa ngân sách và doanh thu', fontsize=16)
    ax.set_xlabel('Ngân sách (USD)', fontsize=12)
...
ax.set_title('Xu hướng số lượng phim qua các năm', fontsize=16)
...
# LƯU HOÀNG
# 1. Biểu đồ Tròn thể hiện trạng thái (status) của phim
st.header("Ngôn ngữ Phim")
status_counts = filtered_data['original_language'].value_counts()
fig, ax = plt.subplots(figsize=(8, 8))
ax.pie(status_counts, labels=status_counts.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette("Set2", len(status_counts)))
ax.set_title('Tỷ lệ ngôn ngữ phim', fontsize=16)
st.pyplot(fig)

# 2. Biểu đồ Đường thể hiện Thời gian chạy (runtime) theo năm
st.header("Thời Gian runtime Theo Năm")
runtime_by_year = filtered_data.groupby('year')['runtime'].mean()
fig, ax = plt.subplots(figsize=(12, 6))
ax.plot(runtime_by_year.index, runtime_by_year.values, marker='o', color='purple', linestyle='-', linewidth=2)
ax.set_title('Thời Gian Chạy Phim Trung Bình Theo Năm', fontsize=16)
ax.set_xlabel('Năm', fontsize=12)
ax.set_ylabel('Thời gian chạy (phút)', fontsize=12)
st.pyplot(fig)

# 3. Biểu đồ Cột Top 10 Quốc Gia Sản Xuất Phim Nhiều Nhất và Thấp Nhất
st.header("Top 10 Quốc Gia Sản Xuất Phim Nhiều Nhất và Thấp Nhất")
...
ax.set_title('Top 10 Quốc Gia Sản Xuất Phim Nhiều Nhất', fontsize=16)
...
if selected_genres:
    ...
    ax.set_title(f"Số Lượng Phim Theo Năm - {', '.join(selected_genres)}")
    ax.set_xlabel("Năm")
    ax.set_ylabel("Số lượng phim")
    st.pyplot(fig)

st.header("So Sánh Doanh Thu và Ngân Sách theo Năm")
...
ax1.set_title('So sánh Ngân sách và Doanh thu theo Năm', fontsize=16)
...
st.header("Biểu đồ Kết Hợp - Độ Phổ Biến và Đánh Giá Phim")
...
fig.suptitle('Sự Phổ Biến và Đánh Giá Phim Theo Năm', fontsize=16)
fig.tight_layout()
st.pyplot(fig)
